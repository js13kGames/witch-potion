    ...
    if (child.type === 'end') {
        if (child.id === 'nextDay') {
            ...
            if (state.day % 7 === 4 ||
                ...
                //   })
                // );
                let dayChild = newEventState.event.children.find(ch => ch.id === 'day');
                if (dayChild) {
...
(function (ResourceType) {
    ...
    ResourceType["EFF_REL"] = "EFFECT_RELOAD";
...
let gameSetupEvents = (state, events) => {
    ...
    let attackEvents = [];
    for (let i = 0; i < 7; i++) {
        let numFireToDefeat = i < 3 ? 1 : i < 6 ? 1 : 2;
    ...
    let finalEvents = [startEventCopy, ...orderedEvents].slice(0, 30);
...
let createBlackCatEvent = (originalBlackCatEvent) => {
    ...
    for (let i = 0; i < potentialRewards.length; i++) {
        let diceEffect = potentialRewards[i];
        event.children.push({
            id: 'dice' + i,
            type: 'm',
    ...
    let catDialog = "\"Perhaps a wise choice.\"";
    event.children.push({
        id: 'pot',
        type: 'm',
        p: catDialog,
...
addEventListener('load', async () => {
    ...
    let calendar = createCalendar(31);
    ...
    gameSetupEvents(gameState, events);
